    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          WhatsApp/SMS/Email campaigns, recall queues and NPS pulse.
        </p>
      </div>
              ...
              <div key={c.id} className="bg-card border border-border rounded-xl p-4 card-lift">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg bg-secondary text-[var(--brand)] flex items-center justify-center">
                      <Icon size={16} />
                    </div>
                    <div>
                      <div className="font-semibold text-sm">{c.name}</div>
                      <div className="text-xs text-muted-foreground">
                        {c.channel} • {c.audience} recipients
                      </div>
                    </div>
                  </div>
                  <Badge
                ...
                <div className="grid grid-cols-3 gap-2 text-center text-xs">
                  <div className="bg-surface rounded-lg p-2">
                    <div className="font-semibold text-base">{c.sent}</div>
                    <div className="text-muted-foreground">Sent</div>
                  </div>
                  <div className="bg-surface rounded-lg p-2">
                    <div className="font-semibold text-base">{c.opened}</div>
                    <div className="text-muted-foreground">Opened</div>
                  </div>
                  <div className="bg-surface rounded-lg p-2">
                    <div className="font-semibold text-base">{c.booked}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 bg-card border border-border rounded-xl shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-border">
            <h3 className="font-display font-semibold">Recall queue</h3>
          </div>
          <div className="divide-y divide-border">
            {recallQueue.map((r) => (
              <div key={r.patientId} className="p-4 flex items-center gap-3">
                ...
                <button className="text-xs px-3 py-1.5 bg-[var(--brand)] text-white rounded-lg hover:bg-[var(--brand-hover)]">
                  Send
                </button>
              </div>
            ))}
            ...
            {npsData.map((n) => (
              <div key={n.rating} className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  <span className="w-2.5 h-2.5 rounded-full" style={{ background: n.color }}></span>
                  {n.rating}
                </div>
