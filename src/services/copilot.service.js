...
const mockSOAP = {
  subjective: 'Patient reports persistent dry cough for 5 days, mild fever (99.4°F) and fatigue. No chest pain. History of seasonal allergies.',
  objective:  'BP 128/82 mmHg, HR 84 bpm, SpO₂ 98%, Temp 99.1°F. Throat mildly erythematous. Chest auscultation clear bilaterally.',
  assessment:'Likely viral upper respiratory tract infection. Differential: early bronchitis, allergic rhinitis flare.',
  plan:      'Symptomatic management. Hydration, rest, paracetamol PRN. Re-evaluate in 72 hrs if symptoms persist.',
};
const mockDx = [
  { code: 'J06.9', label: 'Acute upper respiratory infection', confidence: 0.82 },
...
const mock = {
  generateSOAP:        async () => { await delay(800); return mockSOAP; },
  suggestDiagnosis:    async () => { await delay(700); return mockDx; },
  suggestPrescription: async () => { await delay(700); return mockRx; },
};
const api = {
  generateSOAP:        (p) => apiClient.post('/copilot/soap', p),
  suggestDiagnosis:    (p) => apiClient.post('/copilot/diagnosis', p),
