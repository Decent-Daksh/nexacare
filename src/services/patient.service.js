...
const mock = {
  getAll:       async (params)      => { await delay(); return mockPatients(params); },
  getById:      async (id)          => { await delay(); return mockPatientById(id); },
  getRiskScore: async (id)          => { await delay(); return mockPatientRiskScore(id); },
  create:       async (payload)     => { await delay(600); return { id: 'P-' + Date.now(), ...payload }; },
  update:       async (id, payload) => { await delay(); return { id, ...payload }; },
  delete:       async (id)          => { await delay(); return { success: true, id }; },
};
