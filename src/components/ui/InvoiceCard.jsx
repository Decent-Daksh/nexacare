...
export default function InvoiceCard({ invoice, onDownload, onShare, onView, isLoading = false }) {
  const statusConfig = {
    ...
    <div className="border border-border rounded-lg p-4 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          <h3 className="font-semibold text-sm text-foreground mb-1">{invoice.invoiceNumber}</h3>
          <p className="text-xs text-muted-foreground">{invoice.consultationType}</p>
        </div>
        <Badge variant={invoice.status}>{config.label}</Badge>
      </div>
