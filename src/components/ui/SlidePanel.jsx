...
export default function SlidePanel({ open, onClose, title, children, width = "max-w-md" }) {
  ...
  return (
    <>
      ...
      />
      <aside
        className={`fixed top-0 right-0 h-full w-full ${width} bg-card border-l border-border shadow-2xl z-50 transition-transform duration-300 ${open ? "translate-x-0" : "translate-x-full"}`}
      >
